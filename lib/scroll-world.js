...
export function mountScrollWorld(container, config) {
  let disposed = false, animationId = 0, scrollFrame = 0;
  const abort = new AbortController();
  const reduce = config.reducedMotion ?? window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  ...
  const SECTIONS = (config.sections || []).map(s => ({...s}));
  ...
  SECTIONS.forEach((s, i) => {
    ...
    if (i < N - 1 && CONNECTORS[i]) {
      SEGMENTS.push({ kind: 'conn', si: i, clip: CONNECTORS[i], clipM: CONNECTORS_M[i],
                      still: s.endStill || s.still, accent: SECTIONS[i + 1].accent, w: CONN_W });
    }
  });
  ...
  SEGMENTS.forEach(s => {
    ...
    const img = el('img', 'sw-scene__still'); img.alt = SECTIONS[s.si].label + '微縮校園景觀'; img.decoding = 'async'; img.loading = s === SEGMENTS[0] ? 'eager' : 'lazy';
    if (s.still) img.src = s.still;
    scene.appendChild(img); stage.appendChild(scene);
    scene.dataset.segment = String(SEGMENTS.indexOf(s));
    scene.dataset.kind = s.kind;
    scene.dataset.section = SECTIONS[s.si].id;
  ...
  function jumpTo(i) {
    const seg = SECTIONS[Math.max(0, Math.min(N - 1, i))]._seg;
    window.scrollTo({ top: i === 0 ? 0 : seg.start + (seg.end - seg.start) * 0.5, behavior: reduce ? 'auto' : 'smooth' });
  }

  function loadClip(s) {
    // Under prefers-reduced-motion we never load the clips at all — the stills stay up
    // and simply cross-dissolve as you scroll. No scrubbed video motion, no decode cost.
    if (disposed || reduce || s.loading || s.failed || !s.clip) return;
    s.loading = true;
    // Serve the lighter mobile encode on phones when one was provided.
    const url = (isMobile() && s.clipM) ? s.clipM : s.clip;
    fetch(url, {signal: abort.signal}).then(r => r.ok ? r.blob() : Promise.reject(new Error('404')))
      .then(blob => {
        if (disposed) return;
        ...
        v.addEventListener('error', () => { s.ready = false; s.failed = true; s.el.classList.remove('has-clip'); s.el.dataset.fallback = 'true'; read(); });
        // Reveal the video (hide the still poster) only once a real frame has
        // painted — on iOS a seeked-but-never-played muted video stays blank, so
        // hiding the still on metadata alone would flash an empty scene.
        v.addEventListener('seeked', () => { s.el.classList.add('has-clip'); }, { once: true });
        v.addEventListener('loadeddata', () => { v.pause(); if (!isMobile()) s.el.classList.add('has-clip'); if (userReady) primeVideo(v); });
        s.el.appendChild(v); s.video = v; s.hasClip = true;
      }).catch(() => { if (disposed) return; s.loading = false; s.failed = true; s.el.dataset.fallback = 'true'; read(); });
  }
  ...
  function read() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      const op = i === 0 ? (y <= s.end + fade ? 1 : 0) :
        (y <= s.end + fade ? smooth((y - s.start + fade) / (2 * fade)) : 0);
      s.el.style.opacity = op; s.visible = op > 0.001;
      s.el.style.zIndex = String(i + 1);
      s.el.dataset.target = s.target.toFixed(4);
      s.el.dataset.start = String(s.start);
      s.el.dataset.end = String(s.end);
    ...
    for (let i = 0; i < N; i++) {
      ...
      c.inert = cop <= 0.1;
      c.setAttribute('aria-hidden', String(cop <= 0.1));
    }
    ...
    const pct = Math.round(clamp(y / (totalW * vh)) * 1000) / 10;
    container.dataset.progress = String(pct);
    container.dataset.active = SECTIONS[near].id;
    config.onUpdate?.({index: near, progress: pct, mode: reduce ? 'static' : (cur.failed || !cur.clip ? 'fallback' : cur.ready ? 'ready' : 'loading'), transit: cur.kind === 'conn'});
    ticking = false;
  }
  ...
  function raf(now = performance.now()) {
    if (disposed) return;
    const alpha = 1 - Math.exp(-Math.min(now - lastFrame, 500) / 75);
    lastFrame = now;
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      s.cur += (s.target - s.cur) * (reduce ? 1 : alpha);
  ...
  function onScroll() { if (!ticking) { ticking = true; scrollFrame = requestAnimationFrame(read); } }
  ...
  return {
    jumpTo,
    seekProgress(p) { window.scrollTo({top: clamp(p / 100) * totalW * vh, behavior: 'instant'}); },
    destroy() {
      disposed = true; abort.abort(); cancelAnimationFrame(animationId); cancelAnimationFrame(scrollFrame);
      window.removeEventListener('scroll', onScroll); window.removeEventListener('resize', onResize);
      window.removeEventListener('orientationchange', layout); window.removeEventListener('load', layout);
      window.removeEventListener('pointerdown', onFirstGesture); window.removeEventListener('touchstart', onFirstGesture);
      SEGMENTS.forEach(s => { if (s.video) { s.video.pause(); s.video.removeAttribute('src'); s.video.load(); } if (s.blobUrl) URL.revokeObjectURL(s.blobUrl); });
